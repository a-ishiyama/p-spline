```{r}
library(MASS)
x<-mcycle[,1]
y<-mcycle[,2]
```


```{r}
#Q1

pspline <- function(x,y,k=20,logsp=c(-5,5),bord=3,pord=2,ngrid=100){
  lsp <- seq(logsp[1],logsp[2],length=ngrid)
  edf <- gcv <- rep(0,ngrid)
  
  dk <- diff(range(x))/(k-bord)
  knots <- seq(min(x)-dk*bord,by=dk,length=k+bord+1) 
  X <- splines::splineDesign(knots,x,ord=bord+1,outer.ok=TRUE)
  D <- diff(diag(k),differences=pord)
  
  idenk <- diag(k)
  crossD <- crossprod(D)
  p<-ncol(X);n <-nrow(X)
  QR<-qr(X)
  R<-qr.R(QR)
  
  AR <-forwardsolve(t(R),crossD)
  A <-forwardsolve(t(R), t(AR))
  ep <- eigen(A)
  evl <-diag(ep$values)
  evc <-ep$vectors
  
  for (i in 1:ngrid){
    core <- (idenk + exp(lsp[i])*evl)
    edf[i]<-sum(1/diag(core))
    QRnewcore<-qr(t(R)%*%evc%*%core%*%t(evc)%*%R)
    cholnewcore <-qr.R(QRnewcore)
    beta <- backsolve(cholnewcore, qr.qty(QRnewcore,t(X)%*%y)[1:p])
    y_hat <-X%*%beta
    sigma <- sum((y-y_hat)^2)/(n-edf[i])
    gcv[i]<-sigma/(n-edf[i])
  }
  
  i.opt<-which(gcv==min(gcv))
  core <- (idenk + exp(lsp[i.opt])*evl)
  QRnewcore<-qr(t(R)%*%evc%*%core%*%t(evc)%*%R)
  cholnewcore <-qr.R(QRnewcore)
  beta <- backsolve(cholnewcore, qr.qty(QRnewcore,t(X)%*%y)[1:p])
  y_hat <-X%*%beta
  sigma <- sum((y-y_hat)^2)/(n-edf[i.opt])
  r2 <- 1 - ((n-1)*sigma)/sum((y-mean(y))^2)
  residual <- y-y_hat
  plot(x,y,col="red")
  lines(x,y_hat,col="blue")
  list(b.hat=beta,y.hat=y_hat,sigma=sigma,edf=edf[i.opt],gcv=gcv[i.opt],knots=knots,number.of.coefficients=k, r2=r2, b.order=bord, p.order=pord, residual=residual, x=x, y=y)
}
#Rprof()
demo <-pspline(x,y);demo
#Rprof(NULL)
#summaryRprof()
```

```{r}
#Q2

print.pspline<- function(m){
  print(Effective degree of freedom)
  r2 <- 1 - ((n-1)*sigma)/sum((y-mean(y))^2)
}

```

```{r}
#Q3

predict.pspline <- function(m,x,se=TRUE){
  Xp<- splines::splineDesign(knots=m$knots,x,ord=m$b.order+1,outer.ok=TRUE)
  prediction <- Xp%*%m$b.hat
  if (se==FALSE){
    list(prediction=prediction)
  }else{
    sigma_matrix <- diag(m$sigma,m$number.of.coefficients)
    cholXp <- chol(crossprod(Xp))
    covariance_matrix <-backsolve(cholXp, forwardsolve(t(cholXp),sigma_matrix))
    se <- rowSums(Xp*(Xp%*%covariance_matrix))^0.5
    list(prediction=prediction,se=se)
  }
}

predict.pspline(demo,x)
```

```{r}
#Q4
plot.pspline <- function(m){
  upper <-c(0,length(m$y))
  lower <-c(0,length(m$y))
  for (i in 1:length(m$y)){
    t_value <- qt(0.975, df=length(m$y)-m$number.of.coefficients) #what should be the dof?
    upper[i] <- m$y.hat[i] + t* predict.pspline(m,m$x)$se[i]
    lower[i] <- m$y.hat[i] - t* predict.pspline(m,m$x)$se[i]
  }
  plot(m$x,m$y)
  lines(m$x,m$y.hat)
  lines(m$x,upper)
  lines(m$x,lower)
  
  plot(m$y.hat, m$residual, main="Model residuals against fitted values", xlab="fitted value", ylab="model residuals")
  abline(h=0)
  
  qqnorm(m$residual)
  qqline(m$residual,distribution = qnorm)
  
  list(cbind(ll=lower,yhat=m$y.hat,ul=upper,x=m$x))
}

plot.pspline(demo)
```

```{r}
plot(y~x)
lines(x,demo$y.hat)
#predict.pspline(demo,x)$se[6]
upper <-c(0,length(predict.pspline(demo,x)$prediction))
lower <-c(0,length(predict.pspline(demo,x)$prediction))
for (i in 1:length(predict.pspline(demo,x)$prediction)){
  t <-qt(0.975, df=length(predict.pspline(demo,x)$prediction)-1)
  upper[i] <- predict.pspline(demo,x)$prediction[i] + t*predict.pspline(demo,x)$se[i]
  lower[i] <- predict.pspline(demo,x)$prediction[i] - t*predict.pspline(demo,x)$se[i]
}
lines(x,upper)
lines(x,lower)

```

```{r}
qt(0.975,df=1)
qnorm(0.975)
```

